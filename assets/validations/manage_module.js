    ...
    var handleDetails = function () {
        $('.manage_module_form').validate({
            ...
            rules: {
                mod_name: {
                    required: true,
                    remote: Host + "/module/check_duplicate",
                    lettersonly: true
                },
                mod_portal_id:{
                    required: true,
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: 'POST',
                    url: Host + "/module/saveModuleData",
                    ...
                    success: function (data) {
                        if(data == 'session_timeout')
                        {
                            ...
                            loadDataByPage(1);
                            $('#manageModuleContent').modal('hide');
                            if ($('#mod_id').val() > 0) {
                                showToastMsg("success", "Module updated successfully");
                            } else {
                                showToastMsg("success", "Module added successfully");
                            }
                        }
                        else
                        {
        ...
        $('.manage_module_form input').keypress(function (e) {
            if (e.which == 13) {
                if ($('.manage_module_form').validate().form()) {
                    $.ajax({
                        type: 'POST',
                        url: Host + "/Modules/saveModuleData",
                        data: $('.manage_module_form').serialize(),
                        success: function (data) {
                            if (data == "success") {
                                loadDataByPage(1);
