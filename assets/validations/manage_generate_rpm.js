var Managegeneraterpm = function () {
...
$("#addQuestions").click(function () {
    ...
    $clone.find("#activities_task_subject_text0").attr("id", "activities_task_subject_text" + count).attr("name", "rpm_questions[" + count + "]");
...
function save_generaterpm() {
    $.ajax({
        url: Host + "rpm/save_data",
        type: "POST",
        async: false,
        data: $("#manage_generate_rpm").serialize(),
        success: function (data) {
            if (data == 'success') {
                window.open(HOST + "rpm/generaterpmList", '_self');
            } else {
                showToastMsg("error", "Error in adding rpm and try again.");
            }
            return false;
        },
        statusCode: {
