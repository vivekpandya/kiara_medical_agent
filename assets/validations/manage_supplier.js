    ...
    var handleSupplier = function () {
        $('.manage_supplier_form').validate({
            ...
            rules: {
                ...
                supp_name: {
                    required : "Vendor Name can't left blank"
                },
                supp_phone: {
            ...
            invalidHandler: function (event, validator) { //display error alert on form submit  
                $('.alert-error1', $('.manage_supplier_form')).hide();
                $('.alert-error', $('.manage_supplier_form')).show();
            },
            ...
            submitHandler: function (form) {
                ...
                showloader();
                $(".supplier-submit-popup").prop("disabled","true");
                setTimeout(function () {
                    form.submit();
                    $(".supplier-submit-popup").prop("disabled","false");
                    hideloader();
                }, 1000);
            }
        });

        $('.manage_supplier_form input').keypress(function (e) {
            if (e.which == 13) {
                if ($('.manage_supplier_form').validate().form()) {
                    if ($('#supp_id').val() > 0) {
                        showToastMsg("success", "Data updated successfully");
                    } else {
                        showToastMsg("success", "Data added successfully");
...
function loadDataByPage(page) {
    showloader();
    var supp_first_name = $('#search_supplier').val();
    ...
    $.ajax({
        type: 'POST',
        url: Host + "Supplier/supplier_list",
...
function supplier_print(){
    ...
    $.ajax({
        type: 'POST',
        url: Host + "supplier/supplier_print",
        data: "status=" + status + "&supp_name=" + supp_first_name + "&sort=" + sort + "&colname=" + colname,
        success: function (data) {
            hideloader();
...
$(function () {
    ...
    $("#search_supplier").autocomplete({
        ...
        source: function (request, response) {
            ...
            $.getJSON(Host + "Supplier/search_supplier", request, function (data, status, xhr) {
                cache[ term ] = data;
                response(data);
            });
        },
...
$('#search_supplier').keypress(function (e) {
    if (e.which == 13) {
...
function fill(thisValue) {
    $('#search_supplier').val(unescape(thisValue));
    setTimeout("$('.ui-autocomplete').hide();", 200);
    loadDataByPage(1);
}
...
function deletePhysician(id) {
    $.ajax({
        type: 'POST',
        url: Host + "Supplier/delete_supplier",
        data: "supp_id=" + id,
        success: function (data) {
            $('#showPhysicalDeleteModal').modal('hide');
            loadDataByPage(1);
            showToastMsg("success", "Supplier deleted successfully");
        }
    });
}
