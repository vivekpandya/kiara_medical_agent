var ManageReferralStatus = function () {
    ...
    var handleReferralStatus = function () {
        $('.manage_referral_status_form').validate({
            ...
            rules: {
                status_name : {
                    required : true,
                    remote: {
                        url: Host + "referral_status/check_duplicate_referral_status",
                        type: "POST",
                        data: {
            ...
            invalidHandler: function (event, validator) { //display error alert on form submit  
                $('.alert-error1', $('.manage_referral_status_form')).hide();
                $('.alert-error', $('.manage_referral_status_form')).show();
            },
        ...
        $('.manage_referral_status_form input').keypress(function (e) {
            if (e.which == 13) {
                if ($('.manage_referral_status_form').validate().form()) {
                    if ($('#status_id').val() > 0) {
                        showToastMsg("success", "Data updated successfully");
                    } else {
                        showToastMsg("success", "Data added successfully");
